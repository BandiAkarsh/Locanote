...
test.describe("Note Export", () => {
  ...
  test("can open export modal and see formats", async ({ page }) => {
    // Create a note
    await page.locator('button:has-text("New Note")').or(page.locator('button:has-text("New")')).first().click();
    ...
    // Verify formats
    const markdownOption = page.locator('button.format-option:has-text("Markdown")');
    await expect(markdownOption).toBeVisible();
    await expect(page.locator('button.format-option:has-text("HTML")')).toBeVisible();
    await expect(page.locator('button.format-option:has-text("PDF")')).toBeVisible();
