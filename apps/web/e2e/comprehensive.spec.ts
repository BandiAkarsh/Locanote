...
test.describe("Comprehensive App Test", () => {
  ...
  test("User lifecycle: Register -> Create -> GenUI -> Search -> Export", async ({ page }) => {
    ...
    await page.waitForSelector("#reg-username", { state: "visible", timeout: 30000 });
    ...
    const templateTitleInput = page.locator('input[placeholder="Meeting Notes"]');
    ...
    // Check for Chef Mode
    await expect(page.locator('span:has-text("Chef Mode Active")')).toBeVisible({ timeout: 20000 });
    ...
    await page.keyboard.press("Escape");
    await expect(page.locator('h2:has-text("Export Note")')).not.toBeVisible({ timeout: 20000 });
