...
export function generateChallenge(): ArrayBuffer {
  if (!isBrowser) {
    throw new Error(
      "generateChallenge() can only be called in browser environment",
    );
  }
...
export function generateChallengeBase64(): string {
  if (!isBrowser) {
    throw new Error(
      "generateChallengeBase64() can only be called in browser environment",
    );
  }
...
export function decodeChallenge(base64: string): ArrayBuffer {
  if (!isBrowser) {
    throw new Error(
      "decodeChallenge() can only be called in browser environment",
    );
  }
