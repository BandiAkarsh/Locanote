...
class PerformanceScout {
  ...
  async detectCapabilities() {
    ...
    console.log(
      `[PerformanceScout] Initial Tier: ${this.tier} (GPU: ${this.hasWebGPU}, Memory: ${memory}GB, Cores: ${cores})`,
    );
  }
    ...
    const check = () => {
      ...
      if (now >= lastTime + 1000) {
        ...
        if (this.fps < 30 && this.tier !== "low") {
          console.warn(
            `[PerformanceScout] Low FPS detected (${this.fps}). Downgrading visual tier.`,
          );
          this.tier = "low";
        }
