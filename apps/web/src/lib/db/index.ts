...
// ============================================================================
// This file sets up our browser database using the 'idb' library.
// Think of this as the "database schema" for our application.
//
// WHAT IS INDEXEDDB?
...
interface LocanoteDB extends DBSchema {
  users: {
    ...
    value: User;
    indexes: {                                                  // Indexes for fast lookups
...
// DATABASE INSTANCE
// ============================================================================
// We keep a reference to the open database
...
export async function initDB(): Promise<IDBPDatabase<LocanoteDB>> {
  ...
  db = await openDB<LocanoteDB>('locanote', 1, {                // Open 'locanote' db, version 1
    upgrade(db, oldVersion, newVersion, transaction) {           // Called when creating or upgrading
      ...
      // CREATE USERS STORE
      // --------------------------------------------------------------------
      if (!db.objectStoreNames.contains('users')) {              // Check if store exists
        const userStore = db.createObjectStore('users', {        // Create the store
