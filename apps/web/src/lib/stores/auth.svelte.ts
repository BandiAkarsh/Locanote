...
import type { AuthState, UserSession, AuthResult, AuthError } from '$auth/types';
...
function createAuthStore() {
 ...
 // SET LOADING STATE
  ...
  return {
    ...
    get isAuthenticated() { return state.status === 'authenticated'; },
    get isLoading() { return state.status === 'loading'; },
    get error() { return state.status === 'error' ? state.error : null; },
    get session() { return state.status === 'authenticated' ? state.session : null; },
