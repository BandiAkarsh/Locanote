...
<script lang="ts">
  ...
  // Reactive states for formatting
  // Using $state for individual format flags
  let active = $state({
    bold: false,
    italic: false,
    highlight: false,
  ...
  /**
   * Update active states based on current editor selection
   */
  function updateActiveStates() {
    ...
    active.highlight = editor.isActive('highlight');
    ...
    active.taskList = editor.isActive('taskList');
  ...
  onMount(() => {
    if (editor) {
      // Listen to these events to update the UI
    ...
    voice.onResult = (text, isInterim) => {
      ...
      // STREAMING: Replace previous interim or insert new
      editor.chain()
        .focus()
      ...
      if (isInterim) {
        ...
        // FINAL: Commit and reset
        console.log('[Voice] Final result committed:', text);
        lastInterimLength = 0;
        interimText = '';
      }
    };
  ...
  function btnClass(isActive: boolean, color: 'primary' | 'amber' = 'primary') {
    if (!isActive) return 'text-[var(--ui-text-muted)] hover:bg-primary/10 border-transparent';
    
    if (color === 'amber') {
      return 'bg-amber-500/30 text-amber-500 ring-4 ring-amber-500/20 border-amber-500 scale-110 shadow-[0_0_15px_rgba(245,158,11,0.5)]';
    }
    
    return 'toolbar-btn-active';
  }
</script>
...
<div class="flex items-center gap-1 p-1 sm:p-2 rounded-xl bg-transparent overflow-x-auto scrollbar-hide relative">
  ...
  <div class="flex items-center gap-0.5 px-2 border-r border-[var(--ui-border)]">
    <button onclick={() => editor?.chain().focus().toggleBold().run()} class="p-2 rounded-xl transition-all border-2 {btnClass(active.bold)}" aria-label="Bold"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M6 4h8a4 4 0 014 4 4 4 0 01-4 4H6V4zm0 8h9a4 4 0 014 4 4 4 0 01-4 4H6v-8z" /></svg></button>
    <button onclick={() => editor?.chain().focus().toggleItalic().run()} class="p-2 rounded-xl transition-all border-2 {btnClass(active.italic)}" aria-label="Italic"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg></button>
    <button onclick={() => editor?.chain().focus().toggleHighlight().run()} class="p-2 rounded-xl transition-all border-2 {btnClass(active.highlight, 'amber')}" aria-label="Highlight"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" /></svg></button>
  </div>

  <!-- Headings -->
  <div class="flex items-center gap-0.5 px-2 border-r border-[var(--ui-border)]">
    <button onclick={() => editor?.chain().focus().toggleHeading({ level: 1 }).run()} class="px-2.5 py-1.5 rounded-xl text-xs font-black transition-all border-2 {btnClass(active.h1)}" aria-label="Heading 1">H1</button>
    <button onclick={() => editor?.chain().focus().toggleHeading({ level: 2 }).run()} class="px-2.5 py-1.5 rounded-xl text-xs font-black transition-all border-2 {btnClass(active.h2)}" aria-label="Heading 2">H2</button>
  </div>

  <!-- Lists -->
  <div class="flex items-center gap-0.5 px-2 border-r border-[var(--ui-border)]">
    <button onclick={() => editor?.chain().focus().toggleBulletList().run()} class="p-2 rounded-xl transition-all border-2 {btnClass(active.bulletList)}" aria-label="Bullet List"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M4 6h16M4 12h16M4 18h16" /></svg></button>
    <button onclick={() => editor?.chain().focus().toggleTaskList().run()} class="p-2 rounded-xl transition-all border-2 {btnClass(active.taskList)}" aria-label="Task List"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg></button>
  </div>

  <!-- Blocks -->
  <div class="flex items-center gap-0.5 px-2 border-r border-[var(--ui-border)]">
    {#if !ui.cleanMode}
      <button onclick={() => editor?.chain().focus().toggleCodeBlock().run()} class="p-2 rounded-xl transition-all border-2 {btnClass(active.codeBlock)}" aria-label="Code Block"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg></button>
    {/if}
    <button onclick={() => editor?.chain().focus().toggleBlockquote().run()} class="p-2 rounded-xl transition-all border-2 {btnClass(active.blockquote)}" aria-label="Quote"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg></button>
  </div>
