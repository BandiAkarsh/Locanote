...
<script lang="ts">
  ...
  function animate() {
    ...
    const intentShift = intentStyle
      .getPropertyValue("--intent-bg-shift")
      .trim();
...
<div
  class="fixed inset-0 -z-100 overflow-hidden bg-[var(--ui-bg)] transition-colors duration-1000"
>
  ...
  {:else if currentStyle === "aura"}
    <div class="aura-container absolute inset-0">
      <div
        class="aura-blob aura-1"
        style="background: var(--accent-liquid);"
      ></div>
  ...
  {:else if currentStyle === "crystalline"}
    <div
      class="crystalline-container absolute inset-0"
    ...
    <!-- 4. MOBILE OPTIMIZED (2026 OLED/Mesh Static) -->
  {:else if currentStyle === "static"}
    <div class="absolute inset-0 overflow-hidden">
      <!-- Base Layer (Dark: Deep OLED Black, Light: Soft Gray) -->
      <div class="absolute inset-0 bg-[var(--ui-bg)]"></div>

      <!-- Top Right Mesh (Primary Accent) -->
      <div
        class="absolute -top-[20%] -right-[10%] w-[80vw] h-[80vw] rounded-full mix-blend-screen dark:mix-blend-screen opacity-20 dark:opacity-15 blur-[80px]"
        style="background: radial-gradient(circle, rgba({activeAccentRGB}, 1) 0%, transparent 70%);"
      ></div>

      <!-- Bottom Left Mesh (Secondary/Complementary) -->
      <div
        class="absolute -bottom-[10%] -left-[20%] w-[90vw] h-[90vw] rounded-full mix-blend-screen dark:mix-blend-screen opacity-15 dark:opacity-10 blur-[100px]"
        style="background: radial-gradient(circle, #a855f7 0%, transparent 70%);"
      ></div>

      <!-- Center Subtle Glow -->
      <div
        class="absolute top-[40%] left-[30%] w-[60vw] h-[60vw] rounded-full mix-blend-overlay opacity-10 blur-[60px]"
  ...
  <div
    class="absolute inset-0 opacity-[0.06] pointer-events-none mix-blend-overlay bg-noise"
  ></div>
</div>
...
<style>
  ...
  .crystalline-container {
    background: linear-gradient(
      to bottom,
      var(--ui-bg),
      rgba(var(--accent-liquid-rgb), 0.12)
    );
  }
  ...
  @keyframes shard-float {
    0% {
      transform: translateY(110vh) rotateZ(0deg) rotateX(20deg) scale(0.7);
      opacity: 0;
    }
    10% {
      opacity: var(--opacity);
    ...
    100% {
      transform: translateY(-20vh) rotateZ(360deg) rotateX(-20deg) scale(1.4);
      opacity: 0;
    }
  }
</style>
