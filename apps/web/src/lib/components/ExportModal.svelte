...
<script lang="ts">
	...
	async function handleDownload(format: ExportFormat) {
		...
		try {
			...
			error = err instanceof Error ? err.message : `Failed to export as ${format.toUpperCase()}`;
		} finally {
			isLoading = false;
		}
	}
...
<Modal bind:open title="Export Note" size="large" closeOnBackdrop={true} closeOnEscape={true} onEnter={() => handleDownload(selectedFormat)}>
