...
<script lang="ts">
	...
	import { getAllTemplates, getTemplateCategories, filterTemplates, createNoteFromTemplate } from '$lib/templates/TemplateService.svelte';
	import type { NoteTemplate, TemplateCategory, TemplateFilter } from '$lib/templates/types';
	...
	const iconMap: Record<string, string> = {
		...
		'book-marked': '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/><polyline points="10 2 10 10 13 7 16 10 16 2"/></svg>',
	...
	async function handleCreateNote() {
		...
		isCreating = true;
		try {
			const result = await createNoteFromTemplate(selectedTemplate.id, customTitle);
			if (result.note) {
				// Store template content in sessionStorage so the note page can use it
				if (result.templateContent) {
					sessionStorage.setItem(`template-content-${result.note.id}`, JSON.stringify(result.templateContent));
				}
...
<Modal
	...
	description={showTitleInput ? 'Customize your note title' : 'Select a template to get started quickly'}
...
>
	{#if showTitleInput && selectedTemplate}
		...
		<div class="space-y-4 sm:space-y-5">
			<div class="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-[var(--ui-bg)] rounded-lg">
				<div
				...
				<div class="min-w-0">
					<p class="font-medium text-[var(--ui-text)] text-base sm:text-sm">{selectedTemplate.name}</p>
					<p class="text-sm sm:text-xs text-[var(--ui-text-muted)]">{selectedTemplate.description}</p>
				</div>
			</div>
	...
	{:else}
		...
		<div class="space-y-4 sm:space-y-6">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 max-h-[50vh] sm:max-h-[60vh] overflow-y-auto p-1 -mx-1">
					...
					>
						<div class="flex items-start gap-3 sm:gap-4">
							...
							<div class="flex-1 min-w-0 py-1">
								<h3 class="font-semibold text-[var(--ui-text)] text-base sm:text-sm truncate">
									{template.name}
								</h3>
								<p class="text-sm sm:text-xs text-[var(--ui-text-muted)] line-clamp-2 mt-1 sm:mt-0.5">
			...
			<div class="flex justify-end pt-2 sm:pt-4">
				<Button variant="ghost" onclick={handleCancel} class="min-h-[44px] sm:min-h-0">
					Cancel
				</Button>
			</div>
		</div>
