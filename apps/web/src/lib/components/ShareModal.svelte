  ...
  <div class="space-y-6">
    ...
    <div class="themed-card p-4 bg-primary/5 border-primary/20">
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-xs text-[var(--ui-text-muted)]">
            Require a password to view this note
          </p>
        </div>
        <Toggle
          bind:checked={isPasswordEnabled}
      ...
      {#if isPasswordEnabled}
        <div
          class="space-y-3 animate-in fade-in slide-in-from-top-1 duration-300"
        >
          {#if !isProtected}
          ...
          {:else}
            <div
              class="flex items-center gap-2 text-green-500 text-xs font-bold bg-green-500/10 p-2 rounded-lg"
            >
              <svg
                class="w-4 h-4"
                fill="none"
            ...
            <p class="text-[10px] text-[var(--ui-text-muted)] italic">
              * The encryption key is now hidden from the link. Collaborators
    ...
    {#if hasShareApi}
      ...
      <div class="relative py-2">
        ...
        <div
          class="relative flex justify-center text-xs uppercase tracking-widest font-bold text-[var(--ui-text-muted)]"
        >
          <span class="bg-[var(--ui-surface)] px-2">Social Platforms</span>
        </div>
      </div>
    {/if}
    ...
    <div class="pt-4 border-t border-[var(--ui-border)]">
      <div
        class="flex items-center gap-2 p-2 bg-[var(--ui-bg)] rounded-xl border border-[var(--ui-border)]"
      >
        <input
          type="text"
          readonly
